...
ct_2_qc
ct_3_qc <- subset(ct_3, 
                  subset = nCount_ATAC > 5000 & nCount_ATAC < 40000 &
                  TSS.enrichment > 3 &
                  nucleosome_signal < 1.5)
ct_3_qc
...
obj3_tomerge <- RenameCells(obj3_tomerge, add.cell.id = 3)
...
integrated_modif <- FindClusters(object = integrated, verbose = FALSE, algorithm = 3, resolution = 0.4)
