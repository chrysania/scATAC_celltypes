...
### QC ###
ct_1_qc <- subset(ct_1,
                  subset = nCount_ATAC > 3000 & nCount_ATAC < 50000 &
                  TSS.enrichment > 4 &
                  nucleosome_signal < 1)
ct_1_qc
ct_2_qc <- subset(ct_2,
...
# liver
sample_IDs <- c("ENCSR594BJF-1","ENCSR659ANG-1","ENCSR074DOR-1","ENCSR497ZST-1","ENCSR825WYQ-1","ENCSR118PUH-1","ENCSR650BBI-1","ENCSR552QVU-1","ENCSR630YEA-1","ENCSR139ATR-1")
...
### Integrate ###
obj1_tomerge <- RenameCells(obj1_tomerge, add.cell.id = 3)
...
integrated_modif <- FindClusters(object = integrated, verbose = FALSE, algorithm = 3, resolution = 0.2)
